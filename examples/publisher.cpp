...
void Publisher::publish(const char *message, ssize_t len)
{
	nng_msg *msg;
	nng_msg_alloc(&msg, 0);
	nng_msg_append(msg, message, len);
    nng_sendmsg(_socket, msg, 0);
//    std::cout << std::to_string(len) << " " << std::to_string(ret) << " " << nng_strerror(ret) << std::endl;
}

void Publisher::publish(const std::vector<unsigned char> &message)
{
    publish((char*)message.data(), message.size());
}
